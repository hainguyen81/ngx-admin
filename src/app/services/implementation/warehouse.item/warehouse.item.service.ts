...
import {IWarehouseItem} from '../../../@core/data/warehouse.item';
...
export class WarehouseItemDbService extends AbstractDbService<IWarehouseItem> {
    ...
    getAll(): Promise<IWarehouseItem[]> {
        return super.getAll();
    }

    deleteExecutor = (resolve: (value?: (PromiseLike<number> | number)) => void,
                      reject: (reason?: any) => void, ...args: IWarehouseItem[]) => {
        if (args && args.length) {
...
@Injectable()
export class WarehouseItemHttpService extends AbstractHttpService<IWarehouseItem, IWarehouseItem> {
    ...
    parseResponse(serviceResponse?: ServiceResponse): IWarehouseItem {
        if (!serviceResponse || !serviceResponse.getResponse()
            || !serviceResponse.getResponse().body || !serviceResponse.getResponse().ok) {
            return undefined;
        }
        return JsonUtils.parseResponseJson(serviceResponse.getResponse().body) as IWarehouseItem;
    }
    ...
    handleOfflineMode(url: string, method?: string, res?: any, options?: {
        ...
        errors?: any;
        messages?: any;
    }): Observable<IWarehouseItem[] | IWarehouseItem> {
        return undefined;
    }
}
